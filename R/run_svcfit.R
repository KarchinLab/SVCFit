...
run_svcfit <- function(
    ...
    hemi_dup_r       = 2,      # copies in carrier cells for a hemizygous tandem duplication
    hemi_cn_override = NULL,   # per-chromosome germline CN override; NULL = one copy
    zero_ref_allowlist = NULL  # generated by 13_zero_ref_allowlist.sh; NULL = suppress all
) {
  ...
  if (!is.null(hemizygous_chr) && length(hemizygous_chr)) {
    data$cnv <- apply_hemizygous_cn(data$cnv, hemizygous_chr, hemi_cn_override)
  }
  ...
  svcf <- calc_svcf(
    ...
    exper          = exper,
    hemizygous_chr = hemizygous_chr,
    hemi_cn_bar    = hemi_cn_bar,
    hemi_bg_cn     = hemi_bg_cn,
    hemi_dup_r     = hemi_dup_r,
    zero_ref_allowlist = zero_ref_allowlist
  )
