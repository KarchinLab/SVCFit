...
load_data <- function(p_het, p_onsv, p_sv, p_cnv, chr=NULL, tumor_only) {
  ...
  het_snp <- het_snp %>% mutate(CHROM = paste0("chr", sub("^chr", "", CHROM)))
  if(!is.null(chr)){
  ...
  het_on_sv <- het_on_sv %>% mutate(CHROM = paste0("chr", sub("^chr", "", CHROM)))
  if(!is.null(chr)){
