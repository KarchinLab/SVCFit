...
With `hemi_cn_bar = NULL`, copy-neutral hemizygous SVs still resolve to `SVCF = VAF`, while copy-altered ones are left unresolved (and counted via `svcf_status`) rather than guessed. Autosomes are unaffected: with `hemizygous_chr = NULL` every result is identical to the diploid path.
