...
# Network/Phylogeny
library(igraph)
# Note: Ensure the package containing 'enumerateSpanningTreesModified' is loaded here 
# (e.g., library(clonevol) or source("my_phylogeny_functions.R"))

# --- Global Parameters ---
BASE_DIR    <- "/projects/karchin-lab-hpc/COMBAT/COMBAT_WGS"
LOCAL_DIR   <- "~/Documents/Karchin_lab/combat"
PYTHON_ENV  <- "py3" # Name of your Conda env

# --- Input Files ---
CYTO_FILE   <- file.path(BASE_DIR, "cytoBand.txt")
PAIR_FILE   <- file.path(LOCAL_DIR, "pair.bed")
PURITY_FILE <- file.path(LOCAL_DIR, "samp_pur.csv")

# --- Output Paths ---
OUTPUT_DIR  <- file.path(LOCAL_DIR, "output_figures")
...
```{r}
plot_circos <- function(samp, sv_path, cnv_path, bed_path, cyto_path) {
  ...
  svcfit_dat <- read.delim(bed_path)
...
```{r}
samp = "86921"
sv_name='/projects/karchin-lab-hpc/COMBAT/COMBAT_WGS/svcfit/sv_call/survivor/86921/svcfit_86921.vcf'
cnv_name='/projects/karchin-lab-hpc/COMBAT/COMBAT_WGS/VY_COMBAT_WGS_000_analysis/facets/86921_vs_PBMC_31_FACETS_cncf.txt'
sv_2_cluster <- read.csv("/projects/karchin-lab-hpc/COMBAT/COMBAT_WGS/svcfit/sv_2_cluster.csv")
plot_circus(opts$sample, opts$sv, opts$cnv, has_snp = TRUE)
```

# clustering
```{r}
# read data---------
sample_pair <- read.delim("~/Documents/Karchin_lab/combat/pair.bed", header=FALSE)
colnames(sample_pair)=c("pre_BAT","on_BAT")
samp_pur <- read.delim("~/Documents/Karchin_lab/combat/samp_pur.csv")
...
read_data <- function(pair, row, pur_file){
  ...
  pre <- read.delim(paste0("~/Documents/Karchin_lab/combat/output/", pre_samp, ".bed")) %>%
    mutate(sample_ID=pre_samp,
           pair=row,
           stage='pre_BAT',
           purity=as.numeric(pre_pur))
  on <- read.delim(paste0("~/Documents/Karchin_lab/combat/output/", on_samp, ".bed")) %>%
...
new_plot <- new_plot %>% left_join(cc)
...
write.csv(new_plot, '~/Documents/Karchin_lab/combat/cluster_result.csv')
...
x=check_sv %>% filter(pair==10, cluster_num=='cluster3')
write.csv(y, '~/Documents/Karchin_lab/combat/annotsv/p10_dup.csv', row.names = F)

write.csv(check_sv, '~/Documents/Karchin_lab/combat/annotsv/sv2cluster.csv', row.names = F)
...
```{r}
#get cluster info
cluster_result <- read.csv(paste0(base_dir, "COMBAT/cluster_result.csv"))
