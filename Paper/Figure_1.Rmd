...
mendeley_dir = '~/Documents/mendeley_data'
```

# Fig 1 VISOR benchmark
```{r, message=FALSE}
## can directly load data from /int/extdata/VISOR_bench_data.bed
analyze <- function(samp, exper, p_het, p_onsv, p_sv, p_cnv, p_truth, p_svc, thresh=0.1, chr=NULL, flank_del=50, QUAL_tresh=100, min_alt=2, t_only=FALSE) {
  info <- extract_info(p_het, p_onsv, p_sv, p_cnv, chr_lst=chr, flank_del=flank_del, QUAL_tresh=QUAL_tresh, min_alt=min_alt, tum_only=t_only)
  data <- info[[1]]
  sv_info <- info[[2]]
  snp_df <- info[[3]]
  sv_phase <- info[[4]]
  ...
  svc <- analyze_svclone(samp, exper, final_truth, sv_info, svclone_dir=p_svc)
...
e1 = lapply(condition_list,
             function(x) analyze(x,'exp1',
                                 p_het=file.path(base_dir, 'SNP', 'exp1', paste0("het_near_sv_",x, ".vcf")),
                                 p_onsv=file.path(base_dir, 'SNP', 'exp1', paste0("het_on_sv_",x,".vcf")),
                                 p_sv=file.path(base_dir, 'SV_vcfs', 'exp1', paste0('svt_', x, '.vcf')),
                                 p_cnv=file.path(base_dir, 'facet', 'exp1', paste0(x, '.bed')),
                                 p_truth=file.path(base_dir, 'truth'),
                                 p_svc=file.path(base_dir, 'svclone_rdata', 'exp1', paste0(x, '_ccube_sv_results.RData'))))
e2 = lapply(condition_list,
             function(x) analyze(x,'exp2',
                                 p_het=file.path(base_dir, 'SNP', 'exp2', paste0("het_near_sv_",x, ".vcf")),
                                 p_onsv=file.path(base_dir, 'SNP', 'exp2', paste0("het_on_sv_",x,".vcf")),
                                 p_sv=file.path(base_dir, 'SV_vcfs', 'exp2', paste0('svt_', x, '.vcf')),
                                 p_cnv=file.path(base_dir, 'facet', 'exp2', paste0(x, '.bed')),
                                 p_truth=file.path(base_dir, 'truth'),
                                 p_svc=file.path(base_dir, 'svclone_rdata', 'exp2', paste0(x, '_ccube_sv_results.RData'))))
e3 = lapply(condition_list,
             function(x) analyze(x,'exp3',
                                 p_het=file.path(base_dir, 'SNP', 'exp3', paste0("het_near_sv_",x, ".vcf")),
                                 p_onsv=file.path(base_dir, 'SNP', 'exp3', paste0("het_on_sv_",x,".vcf")),
                                 p_sv=file.path(base_dir, 'SV_vcfs', 'exp3', paste0('svt_', x, '.vcf')),
                                 p_cnv=file.path(base_dir, 'facet', 'exp3', paste0(x, '.bed')),
                                 p_truth=file.path(base_dir, 'truth'),
                                 p_svc=file.path(base_dir, 'svclone_rdata', 'exp3', paste0(x, '_ccube_sv_results.RData'))))
e4 = lapply(condition_list,
             function(x) analyze(x,'exp4',
                                 p_het=file.path(base_dir, 'SNP', 'exp4', paste0("het_near_sv_",x, ".vcf")),
                                 p_onsv=file.path(base_dir, 'SNP', 'exp4', paste0("het_on_sv_",x,".vcf")),
                                 p_sv=file.path(base_dir, 'SV_vcfs', 'exp4', paste0('svt_', x, '.vcf')),
                                 p_cnv=file.path(base_dir, 'facet', 'exp4', paste0(x, '.bed')),
                                 p_truth=file.path(base_dir, 'truth'),
                                 p_svc=file.path(base_dir, 'svclone_rdata', 'exp4', paste0(x, '_ccube_sv_results.RData'))))
e5 = lapply(condition_list,
             function(x) analyze(x,'exp5',
                                 p_het=file.path(base_dir, 'SNP', 'exp5', paste0("het_near_sv_",x, ".vcf")),
                                 p_onsv=file.path(base_dir, 'SNP', 'exp5', paste0("het_on_sv_",x,".vcf")),
                                 p_sv=file.path(base_dir, 'SV_vcfs', 'exp5', paste0('svt_', x, '.vcf')),
                                 p_cnv=file.path(base_dir, 'facet', 'exp5', paste0(x, '.bed')),
                                 p_truth=file.path(base_dir, 'truth'),
                                 p_svc=file.path(base_dir, 'svclone_rdata', 'exp5', paste0(x, '_ccube_sv_results.RData'))))
...
plot_dat <- all_dat %>%
  ...
  summarise(mean_err = mean(true_svcf - svcf, na.rm = TRUE),
            .groups  = "drop") %>%
...
f1a=ggplot(plot_dat, aes(x = mean_err, y = y)) +
  ...
  ylab('')+
  xlab('Mean SVCF estimation error')
...
f1b=all_dat %>%
  ...
  filter(purity %in% c("purity 80%", "purity 40%")) %>%
  ggplot()+
  geom_violin(aes(x=zygosity, y=(svcf-true_svcf), fill = tool))+
  stat_pvalue_manual(f1b_stat, hide.ns = TRUE)+
  ylim(c(0,1.2))+
  ylab('SVCF estimation error')+
  xlab('')+
  theme_minimal()+
  theme(text = element_text(size = 15))
...
analyze <- function(samp, exper, dir, flank_del=50, QUAL_tresh=100, min_alt=2, t_only=FALSE) {
  info <- extract_info(samp, exper, dir, flank_del=flank_del, QUAL_tresh=QUAL_tresh, min_alt=min_alt, tumor_only=t_only)
  data <- info[[1]]
  sv_info <- info[[2]]
  snp_df <- info[[3]]
  sv_phase <- info[[4]]
...
f1c=final %>%
  ...
  ylim(c(-0.5, 0.5))+
  ylab("Mean CCF estimation error")+
  xlab("")+
