...
vignette: "%\\VignetteIndexEntry{SVCFit} %\\VignetteEngine{knitr::rmarkdown}
...
Finally, all of the above functionality is wrapped within a function SVCFit()

# Examples

```{r, echo = FALSE, message=FALSE, fig.width=6, fig.height=3}
single = SVCFit(vcf_path="inst/extdata/c40p50.0s22.bed.vcf",
                tumor_only = TRUE, overlap = TRUE, simulation=FALSE)%>%
  filter(svcf<1.1)

multiple = SVCFit("inst/extdata/c50p50.0s18.bed.vcf",
                tumor_only = TRUE, overlap = TRUE, simulation=FALSE)%>%
  filter(svcf<1.1)
