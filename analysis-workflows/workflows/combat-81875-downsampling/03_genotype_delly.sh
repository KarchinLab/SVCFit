#!/usr/bin/env bash
#SBATCH --job-name=del_svtyper
#SBATCH --output=logs/genotype_delly_%A_%a.out
#SBATCH --error=logs/genotype_delly_%A_%a.err
...
sample="boot${id}"
input="${SV_CALL_ROOT}/delly/${sample}/p_${sample}.vcf"
...
tmp="${out_dir}/d${sample}.vcf"
output="${out_dir}/delly_${sample}.vcf"
for path in "$input" "$bam"; do [[ -f "$path" ]] || { echo "ERROR: missing input: $path" >&2; exit 1; }; done
mkdir -p "$out_dir"

awk 'BEGIN{FS=OFS="\t"} /^#/ {print; next} {info=$8; gsub(/CIPOS=[^;]+;?/,"",info); gsub(/CIEND=[^;]+;?/,"",info); gsub(/MATEID=[^;]+;?/,"",info); $8="CIPOS=-100,100;CIEND=-100,100;MATEID=delly;" info; print}' "$input" > "$tmp"
